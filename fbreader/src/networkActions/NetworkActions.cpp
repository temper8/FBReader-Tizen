...
#include "../optionsDialog/bookInfo/NetworkBookPreviewDialog.h"
#include <FBase.h>

const ZLTypeId NetworkBookDownloadAction::TYPE_ID(ZLTreeAction::TYPE_ID);


NetworkBookPreviewAction::NetworkBookPreviewAction(NetworkBookNode *node) : myNode(node) {
}

void NetworkBookPreviewAction::run() {
	AppLog("NetworkBookPreviewAction::run(");
	if (NetworkBookPreviewDialog(myNode).dialog().run()) {
...
ZLResourceKey NetworkBookPreviewAction::key() const {
	return ZLResourceKey("NetworkBookPreview");
}
...
std::string NetworkBookBuyInBrowserAction::text(const ZLResource &resource) const {
	...
	if (!reference.isNull()) {
		return "BuyInBrowserAction";//ZLStringUtil::printf(text, ((BuyBookReference&)*reference).Price);
	}
	return "text";
}
